...
class TestImageMetadataFromPath:
    ...
    def test_derives_site_code_and_borehole_id_from_folders(self):
        path = Path("/data/GBC/GBC-CB50/GBC-CB50_0015.00-0016.00_vd_p.TIF")
        m = ImageMetadata.from_path(path)
        assert m.site_code == "GBC"
    ...
    def test_parses_borehole_id_with_hyphen(self):
        ...
        m = ImageMetadata.from_path(path)
        assert m.site_code == "Georessourcen"
    ...
    def test_filename_identifier_does_not_need_to_match_borehole_folder(self):
        # LBT-A1W folder but filename starts with A1W — borehole_id still comes from folder
        path = Path("/data/LBT/LBT-A1W/A1W_0000.00-0001.00_vd_p.TIF")
        m = ImageMetadata.from_path(path)
        assert m.site_code == "LBT"
        assert m.borehole_id == "LBT-A1W"
        assert m.depth_start == 0.0
        assert m.depth_end == 1.0

    def test_site_code_with_underscore(self):
        path = Path("/data/LBT_Prognose/LBT-SB92-9B/LBT-SB92-9B_0069.60-0070.50_vd_p.TIF")
        m = ImageMetadata.from_path(path)
        assert m.site_code == "LBT_Prognose"
